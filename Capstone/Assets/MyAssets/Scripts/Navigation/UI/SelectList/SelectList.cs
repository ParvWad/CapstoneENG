...
public class SelectList : MonoBehaviour
{
    ...
    void Start()
    {
        StartCoroutine(WaitForNavController());
    }

    IEnumerator WaitForNavController()
    {
        while (NavigationController.instance == null || NavigationController.instance.agumentedSpace == null)
        {
            yield return null;
        }
        PrepareAllData();
    ...
    void PrepareAllData()
    {
        ...
        foreach (var poi in NavigationController.instance.agumentedSpace.GetPOIs())
        {
            Debug.Log("In SelectList found poi: " + poi);
            pois.Add(poi);
        }
    }
